...
export const ContentView: React.FC<ContentViewProps> = ({
  ...
  const postedProjects = projects; // sab dikhao, slice hata diya (ya rakho agar limit chahiye: .slice(0, 4))

  // Real stats calculated from actual data
  const stats = useMemo(() => {
    const activePostings = projects.filter((p) => p.status === 'open').length;
    const inProduction = projects.filter((p) => p.status === 'in_production').length;
    const completed = projects.filter((p) => p.status === 'completed').length;
    const totalProposals = projects.reduce((sum, p) => sum + (p.proposals || 0), 0);

    return { activePostings, inProduction, completed, totalProposals };
  }, [projects]);

  const statusLabel = (status?: string) => {
    if (status === 'in_production') return { text: 'In Production', style: 'bg-amber-50 text-amber-700 border-amber-200' };
    if (status === 'completed') return { text: 'Completed', style: 'bg-blue-50 text-blue-700 border-blue-200' };
    return { text: 'Active', style: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-xs">
          ...
          <span className="text-xl font-bold text-gray-900">{stats.activePostings}</span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-xs">
          ...
          <span className="text-xl font-bold text-gray-900">{stats.totalProposals}</span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-xs">
          ...
          <span className="text-xl font-bold text-gray-900">{stats.inProduction}</span>
        </div>
        ...
        <div className="divide-y divide-gray-100">
          {postedProjects.length === 0 ? (
            <div className="p-8 text-center text-sm text-gray-500">
              No projects posted yet. Click "Post a New Project" to get started.
            </div>
          ) : (
                  ...
                  <div className="flex items-start gap-3.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold border ${label.style}`}>
                          {label.text}
                        </span>
                      </div>
